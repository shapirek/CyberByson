...
В разные годы в летней школе сотрудники придумывали и порой успешно реализовывали ухищрения, помогавшие упросить технические нюансы работы внутри сезона КЛШ. В 22-м году, например, по утрам выдавалась краткая сводка прошедших планерок, в 23-м году техно-зондера собирали информацию о необходимой технике с помощью системы записок, в 24-м году сообщения о звонках от родителей передавались напрямую вожатым через телеграмм (вручную Викой Луковской). С неизбежным уходом сотрудников из жизни КЛШ исчезают и следы их трудов и инициатив — поэтому мне и хотелось, чтобы их дело без устали продолжала машина, в нашем случае бот. 
...
- **Запросить технику**: оставьте сообщение для всех технозондеров о необходимости получить технику — технозондера смогут обсудить Вашу заявку между собой (например, договориться, кто отнесет технику).
...
Запускаем бота с помощью команды /start

![Запуск бота](assets/Запуск_КиберБизон.gif)

## Функционал для родителей

* **"Пусть ребенок мне позвонит!"**
  
Родитель вписывает имя и фамилию школьника (можно в любом порядке, можно опечататься) -- и бот его находит в соответствующей гугл-таблице. Если он находит несколько похожих вариантов (например, если среди школьников есть тезки), то предлагает выбрать, какой конкретный школьник нужен.

![Пусть ребенок мне позвонит!](assets/Пусть_ребенок_мне_позвонит_КиберБизон.gif)

## Функционал для сотрудников

* **Вход по паролю**

Для доступа к меню сотрудников необходимо ввести пароль -- бот сверяется с данными в соответствующей таблице. Ваши сообщения будут подписываться Вашим именем! 

![Вход по паролю](assets/Вход_по_паролю_КиберБизон.gif)

* **НОВОСТИ**

Сотрудник может узнать краткую выжимку последней прошедшей планерки, касающейся текущего дня!

![НОВОСТИ](assets/НОВОСТИ_КиберБизон.gif)

* **Запрос техники**

Сотрудник может оставить запрос на получение техники, указав необходимые детали в сообщении -- бот отошлет всем сотрудникам, помеченным как "технозондера" в соответствующей таблице. Получатели могут нажать на кнопку "Подставить техноплечо" -- их перекинет в секцию комментариев к этому сообщению. 

![Запросить технику](assets/Запросить_технику_КиберБизон.gif)

* **Выдать наряд**

Сотрудник может оставить запрос на выдачу наряда конкретному школьнику (не страшно, если сотрудник ошибется в имени), указав необходимые детали в сообщении -- бот отошлет всем вожатым, помеченным как сотрудники "ПСИН" в соответствующей таблице. Обратной связи не предусмотрено, запросы *не* анонимные. 

![Выдать наряд](assets/Выдать_наряд_КиберБизон.gif)

* **Написать сообщение**

Сотрудник может отправить сообщение группе получателей: дирекции, направлению, судьям турниров (можно выбрать), вожатым команды (можно выбрать), всем сотрудникам КЛШ. 

Это может быть просто объявление, не требующее ответа:

![Сообщение без ответа](assets/Сообщение_без_ответа_КиберБизон.gif)

И это сообщение может подразумевать наличие ответа. Тогда отправитель получит копию сообщения с кнопками "Ответы" и "Проблема решена!". При нажатии на кнопку "Ответы" его перекинет в комментарии к этому сообщению, при нажатии на кнопку "Проблема решена!" все сообщения удалятся -- и из закрепа, и из соответствующего канала, так что к комментариям будет перейти нельзя. 

![Сообщение с ответом](assets/Сообщение_с_ответом_КиберБизон.gif)

Получатели сообщения, которое подразумевает ответ, получат сообщение с дополнительной кнопкой "Подставить плечо", при нажатии на которую перекидывает к комментариям к сообщению в соответствующем канале. Там можно и реакции других получателей посмотреть, и на вопрос ответить. 

![Подставить плечо](assets/Подставить_плечо_КиберБизон.gif)

* **Бонус!**

Сообщения от дирекции выглядят иначе -- они выделены красными восклицательными знаками. 

![Сообщение от дирекции](assets/Бонус_Сообщение_от_дирекции_КиберБизон.png)
